...
regex       =re.compile('.* Iteration (\d+), Testing net \(#\d+\)')
regexLos    =re.compile('.*Test net output #0: error = (%s)'%(floatReg,))
...
f = open(logFilePath, 'r').readlines()
#with open(logFilePath, 'r').readlines() as f:
global result
print len(f)
for line in f:
